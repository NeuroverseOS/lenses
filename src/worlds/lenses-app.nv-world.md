...
- `perspective_only` — Every AI response must reframe through the user's chosen philosophical perspective. Lenses is a perspective companion, not a signal detector. The AI must never detect, classify, or label behavioral signals (deception, deflection, inconsistency, emotional states). That is the domain of other apps. Lenses sees the world through a philosophy — it does not diagnose the world. (structural, immutable)
- `no_signal_detection` — The AI must never output behavioral classifications, confidence scores, signal labels, or pattern detection results. No "they seem defensive," no "inconsistency detected," no "deception probability." If the user asks "are they lying?" the lens reframes through philosophy ("What would change if they were?"), never through analysis. (structural, immutable)
